<template>
  <div>
    ...
    <van-row gutter="5">
      <van-col span="8" v-for="item in recommendSongList" :key="item.id">
...
<script>
import { getRecommendSongListApi, getNewSongListApi } from "@/api";
export default {
  data() {
    return {
      recommendSongList: [],
  ...
  methods: {
    async getRecommendSongList() {
      const res = await getRecommendSongListApi({ limit: 6 });
